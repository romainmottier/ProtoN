...
#include "methods/hho"
#include "methods/cuthho" // candidate for deletion
...
int main(int argc, char **argv)
{
    ...
    auto exact_sol_fun = [](const typename poly_mesh<RealType>::point_type& pt) -> RealType {
        return std::sin(M_PI*pt.x()) * std::sin(M_PI*pt.y());
    };
    auto exact_grad_sol_fun = [](const typename poly_mesh<RealType>::point_type& pt) -> Matrix<RealType, 1, 2> {
        Matrix<RealType, 1, 2> grad;
        grad(0,0) = M_PI * std::cos(M_PI*pt.x()) * std::sin(M_PI*pt.y());
        grad(0,1) = M_PI * std::sin(M_PI*pt.x()) * std::cos(M_PI*pt.y());
        return grad;
    };
...
//    std::cout << "dof = " << std::endl <<  alpha_dof << std::endl;
    tc.toc();
...
//    auto fbs = face_basis<poly_mesh<RealType>,RealType>::size(k_degree);
