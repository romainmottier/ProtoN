...
double ComputeEnergy(poly_mesh<double> & msh, hho_degree_info & hho_di, assembler<poly_mesh<double>> & assembler, Matrix<double, Dynamic, 1> & p, Matrix<double, Dynamic, 1> & v);
...
int main(int argc, char **argv)
{
    ...
    a_dof_n = analysis.solve(mass_assembler_a.RHS); // Initial acceleration
        ...
        RealType energy_h0 = ComputeEnergy(msh, hho_di, mass_assembler_a, p_dof_n, v_dof_n);
    ...
    if (is_implicit_Q) {
        ...
        for(size_t it = 1; it <= nt; it++){
            ...
            {
                tc.tic();
                RealType energy_h = ComputeEnergy(msh, hho_di, assembler, p_dof_n, v_dof_n);
                tc.toc();
        ...
        #ifdef compute_energy_Q
            std::cout << bold << cyan << "Reporting energy values : " << energy_h_values << reset << std::endl;
            std::cout << bold << cyan << "Reporting initial energy value : " << energy_h0 << reset << std::endl;
        #endif
...
double ComputeEnergy(poly_mesh<double> & msh, hho_degree_info & hho_di, assembler<poly_mesh<double>> & assembler, Matrix<double, Dynamic, 1> & p_dof_n, Matrix<double, Dynamic, 1> & v_dof_n){
    using RealType = double;
