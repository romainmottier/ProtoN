...
public:
    ...
    static void write_silo_grad_poly_ext(std::string silo_file_name, size_t it, Mesh & msh, 
    hho_degree_info & hho_di, Matrix<double, Dynamic, 1> &grad_dof, testType &test_case,
    one_field_interface_assembler<Mesh, std::function<double(const typename Mesh::point_type& )>> & assembler) {
        ...
        Rec_Gx.reserve(num_points);
        Rec_Gy.reserve(num_points);
        ...
        for (auto& node : msh_nodes) {
            ...
            for (auto& cell : cells ) {
                ...
                auto offset_cl = cell_table.at(offset(msh, cl));
                if (!is_cut(msh, cl)) {
                    // Gradient reconstruction evaluation
                    ...
                    auto vec_t_phi = vector_cell_basis.eval_basis(pt);
                    Eigen::VectorXd t_phi_x = vec_t_phi.col(0); 
                    Eigen::VectorXd t_phi_y = vec_t_phi.col(1);  
                    rec_gx += grad_dofs.dot(t_phi_x);
                    rec_gy += grad_dofs.dot(t_phi_y);
                }
                else {
            ...
            Rec_Gx.push_back(rec_gx);
            Rec_Gy.push_back(rec_gy);
        }
        ...
        silo.create(silo_file_name.c_str());
        silo.add_mesh(msh, "mesh");
        silo.add_variable("mesh", "1_Gx",  exact_Gx.data(),  exact_Gx.size(), nodal_variable_t);
        silo.add_variable("mesh", "2_Gy",  exact_Gy.data(),  exact_Gy.size(), nodal_variable_t);
        silo.add_variable("mesh", "3_Gxh", Rec_Gx.data(), Rec_Gx.size(), nodal_variable_t);
        silo.add_variable("mesh", "4_Gyh", Rec_Gy.data(), Rec_Gy.size(), nodal_variable_t);
        silo.close();
        tc.toc();
        std::cout << bold << yellow << "         Silo file - GRADIENT - rendered in : " << tc << " seconds" << reset << std::endl;
    }
    
    // template<typename testType>
    // static void write_silo_grad_poly_ext_centered(std::string silo_file_name, size_t it, Mesh & msh, 
    // hho_degree_info & hho_di, Matrix<double, Dynamic, 1> &grad_dof, 
    // testType &test_case,
    // one_field_interface_assembler<Mesh, std::function<double(const typename Mesh::point_type& )>> & assembler) {
    ...
    //     silo.close();
    //     tc.toc();
    //     std::cout << bold << yellow << "         Silo file - GRADIENT CENTERED - rendered in : " << tc << " seconds" << reset << std::endl;
    // }
    
    // template<typename testType>
    // static void write_silo_one_field_poly_ext_centered(std::string silo_file_name, size_t it, Mesh & msh, 
    ...
    template<typename testType>
    static void write_silo_one_field_poly_ext(std::string silo_file_name, size_t it, Mesh & msh, 
    hho_degree_info & hho_di, Matrix<double, Dynamic, 1> &x_dof, testType &test_case,
    one_field_interface_assembler<Mesh, std::function<double(const typename Mesh::point_type& )>> & assembler) {
    ...
    template<typename testType>
    static void write_silo_one_field_extension(std::string silo_file_name, size_t it, Mesh & msh, 
    hho_degree_info & hho_di, Matrix<double, Dynamic, 1> &x_dof, Matrix<double, Dynamic, 1> &grad_dof, 
    testType &test_case,
    one_field_interface_assembler<Mesh, std::function<double(const typename Mesh::point_type& )>> & assembler) {
        ...
        silo.add_variable("mesh", "5_Gxh", approx_Gx.data(), exact_Gx.size(), nodal_variable_t);
        silo.add_variable("mesh", "6_Gyh", approx_Gy.data(), exact_Gy.size(), nodal_variable_t);
        silo.add_variable("mesh", "7_Rec_Gxh", Rec_Gx.data(), Rec_Gx.size(), nodal_variable_t);
        silo.add_variable("mesh", "8_Rec_Gyh", Rec_Gy.data(), Rec_Gy.size(), nodal_variable_t);
        silo.close();
        tc.toc();
...
//                        }
        ...
        if (cell_centered_Q) {
            ...
            silo.add_variable("mesh", "Gx", exact_Gx.data(), exact_Gx.size(), zonal_variable_t);
            silo.add_variable("mesh", "Gy", exact_Gy.data(), exact_Gy.size(), zonal_variable_t);
        }else{
...
//                        }
    ...
    // /// Compute L2 and H1 errors for one field approximation
    // static void compute_errors_one_field_bis(Mesh & msh, hho_degree_info & hho_di, one_field_interface_assembler<Mesh, std::function<double(const typename Mesh::point_type& )>> & assembler, Matrix<double, Dynamic, 1> & x_dof,std::function<double(const typename Mesh::point_type& )> scal_fun, std::function<Matrix<double, 1, 2>(const typename Mesh::point_type& )> flux_fun, std::ostream & error_file = std::cout){
    ...
    //        if ( location(msh, cell) == element_location::ON_INTERFACE ) {
    //            {// NEGATIVE SIDE 
    //                cut_cell_basis<cuthho_poly_mesh<RealType>, RealType> cell_basis(msh, cell, hho_di.cell_degree(), element_location::IN_NEGATIVE_SIDE);
    //                auto cbs = cell_basis.size();  
    ...
    //            }
    //            {   // POSITIVE SIDE 
    //                cut_cell_basis<cuthho_poly_mesh<RealType>, RealType> cell_basis(msh, cell, hho_di.cell_degree(), element_location::IN_POSITIVE_SIDE);
    //                auto cbs = cell_basis.size();  
    ...
    //        }
    //        else {
    //            cut_cell_basis<cuthho_poly_mesh<RealType>, RealType> cell_basis(msh, cell, hho_di.cell_degree(), location(msh, cell));
    //            auto cbs = cell_basis.size();  
    ...
    //    for (auto& cell : msh.cells) {
    //        l2_error_vec[cell_i] = 0.0;
    //        flux_l2_error_vec[cell_i] = 0.0;
    //        RealType h_l = diameter(msh, cell);
    //        if (h_l < h) 
    //            h = h_l;
    //        if ( location(msh, cell) == element_location::ON_INTERFACE ) {
    //            // DISCRETIZATION INFOS NEGATIVE SIDE
    //            cut_cell_basis<cuthho_poly_mesh<RealType>, RealType> neg_cell_basis(msh, cell, hho_di.cell_degree(), element_location::IN_NEGATIVE_SIDE);
    //            auto dofs_n = assembler.take_local_data(msh, cell, x_dof, element_location::IN_NEGATIVE_SIDE);
    ...
    //            cut_cell_basis<cuthho_poly_mesh<RealType>, RealType> pos_cell_basis(msh, cell, hho_di.cell_degree(), element_location::IN_POSITIVE_SIDE);
    //            auto dofs_p = assembler.take_local_data(msh, cell, x_dof, element_location::IN_POSITIVE_SIDE);
    //            auto cell_dofs_p = dofs_p.head(cbs);
    //            auto qps_p = integrate(msh, cell, 2*hho_di.cell_degree(), element_location::IN_POSITIVE_SIDE);
    //            for (auto& qp : qps_p) {
    ...
    //            auto cell_dofs = dofs.head(cbs);
    //            // uncut case
    //            auto qps = integrate(msh, cell, 2*hho_di.cell_degree(), location(msh, cell));
    //            for (auto& qp : qps) {
    //                /* Compute H1-error */
    //                auto t_dphi = cell_basis.eval_gradients( qp.first );
    //                Matrix<RealType, 1, 2> grad = Matrix<RealType, 1, 2>::Zero();
    ...
    // static void compute_errors_grad_one_field_centered(Mesh & msh, hho_degree_info & hho_di, one_field_interface_assembler<Mesh, std::function<double(const typename Mesh::point_type& )>> & assembler, Matrix<double, Dynamic, 1> & grad_dof, std::function<Matrix<double, 1, 2>(const typename Mesh::point_type& )> flux_fun, std::ostream & error_file = std::cout){
    ...
    static void compute_errors_grad_one_field(Mesh & msh, hho_degree_info & hho_di, one_field_interface_assembler<Mesh, std::function<double(const typename Mesh::point_type& )>> & assembler, Matrix<double, Dynamic, 1> & grad_dof, std::function<Matrix<double, 1, 2>(const typename Mesh::point_type& )> flux_fun, std::ostream & error_file = std::cout){
    ...
    // Write Python file to plot convergence tests
    static void write_conv_grad_grad(const std::string& txtFilename) {
    ...
    // Write python file to plot convergence tests 
    static void write_conv_grad(const std::string& txtFilename) {
