...
public:
    ...
    Mat
    make_contrib_grad(const Mesh& msh, const typename Mesh::cell_type& cl, const testType &test_case, const hho_degree_info hdi) {
        if( location(msh, cl) != element_location::ON_INTERFACE )
            return uncut_method<T, ET, testType>::make_contrib_uncut_grad(msh, cl, hdi, test_case);
        else 
            return cut_method<T, ET, testType>::make_contrib_cut_grad(msh, cl, test_case, hdi);
    }
