...
class test_case_laplacian_waves: public test_case_laplacian<T, Function, Mesh> {
   ...
   public:
       ...
       test_case_laplacian_waves(T t,Function level_set__) : test_case_laplacian<T, Function, Mesh> (level_set__, params<T>(),
           // SOLUTION
           [level_set__,t](const typename Mesh::point_type& pt) -> T { 
               if(level_set__(pt) > 0)
            ...
            [level_set__,t](const typename Mesh::point_type& pt) -> T { 
                ...
                if(level_set__(pt) > 0)
                    ...
                    return (1 - pt.x())*pt.x()*(1 - pt.y())*pt.y()*std::sin(std::sqrt(2.0)*M_PI*t);},
            // GRAD
            [level_set__,t](const typename Mesh::point_type& pt) -> auto { 
                Matrix<T, 1, 2> ret;
                T x,y;
                x = pt.x();
                y = pt.y();
...
newmark_step_cuthho_interface(size_t it, double  t, typename Mesh::coordinate_type dt, typename Mesh::coordinate_type beta, typename Mesh::coordinate_type gamma, Mesh& msh, hho_degree_info & hdi, meth &method, testType &test_case, Matrix<double, Dynamic, 1> & u_dof_n, Matrix<double, Dynamic, 1> & v_dof_n, Matrix<double, Dynamic, 1> & a_dof_n, SparseMatrix<typename Mesh::coordinate_type> & Kg, linear_solver<typename Mesh::coordinate_type> & analysis) {
    ...
    if (u_dof_n.rows() == 0) {
        ...
        // SOLUTION AND ITS TIME DERIVATIVES 
...
void ICutHHOSecondOrderConvTest(int argc, char **argv);
void ICutHHOSecondOrderConvTest(int argc, char **argv) {
    ...
    sim_infos << std::endl << "   CONVERGENCE TEST ON SECOND ORDER WAVE PROBLEM - POLYNOMIAL EXTENSION";
    ...
    for(size_t k = degree; k <= degree; k++){
        ...
        for(size_t l = 0; l <= l_divs; l++){
            ...
            analysis.factorize();
            sim_infos << "Number of equations : " << analysis.n_equations() << std::endl;
            sim_infos << "Number of steps : " <<  nt << std::endl;
            sim_infos << "Time step size : " <<  dt << std::endl;
            ...
            std::cout << bold << yellow << "         Time discretization:" << reset << std::endl; 
            for(size_t it = 1; it <= nt; it++) { 
                ...
                if ((it == 1) || (it == std::round(nt/3)) || (it == std::round(nt/2)) || (it == std::round(3*nt/2)) || (it == nt)) 
                    std::cout << bold << "            Time step number " << it << ": " << t << "seconds" << reset << std::endl; 
...
int main(int argc, char **argv) {
    ICutHHOSecondOrderConvTest(argc, argv);
    return 0;
}
